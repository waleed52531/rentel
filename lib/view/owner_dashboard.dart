...
import 'package:rent_settlement_app/bloc/owner/maintenance/owner_maintenance_bloc.dart';
...
import 'package:rent_settlement_app/bloc/owner/monthly_records/owner_monthly_records_bloc.dart';
import 'package:rent_settlement_app/bloc/owner/monthly_records/owner_monthly_records_event.dart';
import 'package:rent_settlement_app/bloc/owner/monthly_records/owner_monthly_records_state.dart';
import 'package:rent_settlement_app/bloc/owner/properties/owner_properties_bloc.dart';
import 'package:rent_settlement_app/bloc/owner/properties/owner_properties_event.dart';
import 'package:rent_settlement_app/bloc/owner/properties/owner_properties_state.dart';
import 'package:rent_settlement_app/bloc/owner/properties/owner_property_detail_bloc.dart';
...
import 'package:rent_settlement_app/bloc/owner/tenancies/owner_tenancies_bloc.dart';
import 'package:rent_settlement_app/bloc/owner/tenancies/owner_tenancies_event.dart';
import 'package:rent_settlement_app/bloc/owner/tenancies/owner_tenancies_state.dart';
              ...
              child: ListView(
                  ...
                  children: [
                    RentraDashboardHeader(
                      ...
                      subtitle:
                          'Publish listings, review availability, and keep every rental asset ready.',
                      ...
                      metrics: [
                        ...
                        RentraMetricData(
                            label: 'Published',
                            value:
                                '${items.where((item) => item.publicationStatus == PublicationStatus.published).length}',
                            icon: Icons.public_outlined),
                        RentraMetricData(
                            label: 'Available',
                            value:
                                '${items.where((item) => item.status == PropertyStatus.available).length}',
                    ...
                    if (items.isEmpty)
                      ...
                      const RentraSectionTitle(
                        ...
                        subtitle: 'Operational health across your listings.',
                      ),
                      ...
                      RentraQuickActionStrip(actions: [
                        ...
                        RentraQuickAction(
                          ...
                          value:
                              '${items.fold<int>(0, (sum, item) => sum + item.images.length)}',
                          label: 'Photos',
                        ),
                      ]),
                      const SizedBox(height: 16),
                      const RentraSectionTitle(
                        ...
                        subtitle: 'Tap any listing to edit details or media.',
                      ),
                      const SizedBox(height: 10),
        ...
        if (items.isEmpty) {
          return ListView(padding: const EdgeInsets.all(16), children: const [
            RentraDashboardHeader(
              ...
              subtitle: 'Review renter interest and convert accepted leads.',
        ...
        return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(16), children: [
              RentraDashboardHeader(
                title: 'Application review',
                subtitle:
                    'Prioritize applicants, respond quickly, and create tenancies.',
                icon: Icons.assignment_outlined,
                metrics: [
                  RentraMetricData(
                      label: 'Total',
                  ...
                  RentraMetricData(
                      label: 'Pending',
                      value:
                          '${items.where((item) => item.status == ApplicationStatus.pending || item.status == ApplicationStatus.reviewed).length}',
                      icon: Icons.rate_review_outlined),
                  RentraMetricData(
                      label: 'Accepted',
                      value:
                          '${items.where((item) => item.status == ApplicationStatus.accepted).length}',
                      icon: Icons.check_circle_outline),
                ],
              ),
              const SizedBox(height: 16),
              ...items.map((item) => ApplicationCard(
                    application: item,
                    footer: _applicationFooter(context, item),
                  ))
            ]));
      });

  Widget? _applicationFooter(BuildContext context, RentalApplication item) {
      ...
      return Row(children: [
        Expanded(
            child: OutlinedButton(
                onPressed: () =>
                    _decide(context, item, ApplicationStatus.rejected),
                child: const Text('Reject'))),
        const SizedBox(width: 8),
        Expanded(
            child: FilledButton(
                onPressed: () =>
                    _decide(context, item, ApplicationStatus.accepted),
      ...
      return Align(
          ...
          child: FilledButton.tonalIcon(
              onPressed: () => context
                  ...
                  .add(OwnerTenancyAssigned(item.id, DateTime.now())),
              icon: const Icon(Icons.key),
              label: const Text('Create tenancy')));
    }
    return null;
  }
        ...
        if (items.isEmpty) {
          return ListView(padding: const EdgeInsets.all(16), children: [
            RentraDashboardHeader(
              ...
              subtitle: 'Create renter access and manage rental terms.',
        ...
        return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(16), children: [
              RentraDashboardHeader(
                title: 'Tenancy control',
                subtitle:
                    'Manage active leases, billing terms, and renter assignments.',
                icon: Icons.key_outlined,
                action: FilledButton.icon(
                    onPressed: () => _create(context),
                    icon: const Icon(Icons.person_add_outlined),
                ...
                metrics: [
                  ...
                  RentraMetricData(
                      label: 'Active',
                      value:
                          '${items.where((item) => item.status == TenancyStatus.active).length}',
                      icon: Icons.check_circle_outline),
                ],
              ),
              const SizedBox(height: 16),
        ...
        if (items.isEmpty) {
          return ListView(padding: const EdgeInsets.all(16), children: const [
            RentraDashboardHeader(
              ...
              subtitle:
                  'Approve, reject, reopen, and audit monthly rent flows.',
        ...
        return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(16), children: [
              RentraDashboardHeader(
                title: 'Payment records',
                subtitle:
                    'Review renter submissions and keep frozen history clean.',
                icon: Icons.receipt_long_outlined,
                metrics: [
                  RentraMetricData(
                      label: 'Records',
                  ...
                  RentraMetricData(
                      label: 'Pending',
                      value:
                          '${items.where((item) => item.status == MonthlyRecordStatus.pending).length}',
                      icon: Icons.pending_actions_outlined),
                  RentraMetricData(
                      label: 'Approved',
                      value:
                          '${items.where((item) => item.status == MonthlyRecordStatus.approved).length}',
                      icon: Icons.verified_outlined),
                ],
              ),
              const SizedBox(height: 16),
              ...items.map((item) => MonthlyRecordCard(
        ...
        if (items.isEmpty) {
          return ListView(padding: const EdgeInsets.all(16), children: const [
            RentraDashboardHeader(
              ...
              subtitle: 'Track renter issues and move requests through status.',
        ...
        return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(16), children: [
              RentraDashboardHeader(
                title: 'Maintenance operations',
                subtitle:
                    'Track renter issues, attachments, and service progress.',
                icon: Icons.build_outlined,
                metrics: [
                  RentraMetricData(
                      label: 'Requests',
              ...
              ...items.map((item) => MaintenanceCard(
                    ...
                    footer: item.allowedTransitions.isEmpty
                        ? null
                        : Wrap(
                            spacing: 8,
                            runSpacing: 8,
                            children: item.allowedTransitions
                                .map((status) => FilledButton.tonal(
                                    onPressed: () => context
                                        .read<OwnerMaintenanceBloc>()
                                        .add(OwnerMaintenanceStatusChanged(
                                            item.id, status)),
